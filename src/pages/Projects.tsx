...
export default function Projects() {
  ...
  return (
    <div className="flex h-full">
      {/* Project List */}
      <div className="w-72 shrink-0 border-r border-[#1a2035] flex flex-col">
        <div className="p-4 border-b border-[#1a2035] flex items-center justify-between">
          <h1 className="text-base font-bold text-slate-100">Projects</h1>
          <button onClick={openAdd} className="btn-primary py-1.5 px-2.5 flex items-center gap-1 text-xs">
            <Plus size={12} /> New
          </button>
        </div>
        <div className="flex-1 overflow-y-auto">
          {projects.length === 0 && <p className="text-slate-600 text-xs text-center py-8">No projects yet.</p>}
          {projects.map(p => {
            ...
            return (
              <div
                ...
                className={`p-4 border-b border-[#1a2035] cursor-pointer hover:bg-[#111829] transition-colors ${selected === p.id ? "bg-[#111829] border-l-2 border-l-indigo-500" : ""}`}
              >
                <div className="flex items-start justify-between">
                  <p className="text-sm font-medium text-slate-200">{p.name}</p>
                  <span className={`badge text-[10px] ${STATUS_BADGE[p.status]}`}>{p.status}</span>
                </div>
                {course && <p className="text-xs text-slate-600 mt-0.5">{course.code}</p>}
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="text-[10px] text-slate-600">{prog}%</span>
                </div>
                {p.deadline && <p className="text-[10px] text-slate-600 mt-1">Due {format(parseISO(p.deadline), "MMM d")}</p>}
              </div>
            );
          })}
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {!selectedProject ? (
          ...
          <>
            {/* Header */}
            <div className="p-5 border-b border-[#1a2035]">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-lg font-bold text-slate-100">{selectedProject.name}</h2>
                  <p className="text-slate-500 text-sm mt-0.5">{selectedProject.description}</p>
                </div>
              ...
              <div className="flex items-center gap-4 mt-3">
                ...
                {selectedProject.deadline && <span className="text-xs text-slate-500">Deadline: {format(parseISO(selectedProject.deadline), "MMM d, yyyy")}</span>}
            ...
            <div className="flex-1 overflow-y-auto p-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                {(["todo", "in-progress", "done"] as TaskStatus[]).map(status => {
                  ...
                  return (
                    <div key={status} className="bg-[#0d1120] rounded-xl border border-[#1a2035]">
                      <div className="p-3 border-b border-[#1a2035] flex items-center justify-between">
                        <span className="text-xs font-semibold text-slate-400 uppercase">{status.replace("-", " ")}</span>
                        ...
                        {colTasks.map(t => (
                          <div key={t.id} className="card group p-3 hover:border-indigo-600/30 transition-colors">
                            <div className="flex items-center gap-2">
                              ...
                              <p className={`text-sm flex-1 ${t.status === "done" ? "line-through text-slate-600" : "text-slate-300"}`}>{t.title}</p>
                              <div className="hidden group-hover:flex gap-1">
                                <button onClick={() => openEditTask(t)} className="text-slate-500 hover:text-slate-300"><Edit2 size={11} /></button>
                            ...
                            {(t.assignedTo || t.dueDate) && (
                              <div className="flex items-center gap-2 mt-1.5 pl-5">
                                {t.assignedTo && <span className="text-[10px] text-slate-600">{t.assignedTo}</span>}
                                {t.dueDate && <span className="text-[10px] text-slate-600 ml-auto">{format(parseISO(t.dueDate), "MMM d")}</span>}
                              </div>
                            )}
                          </div>
                        ))}
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {form.members.map(m => (
                <span key={m} className="badge bg-slate-800 text-slate-300 flex items-center gap-1">
                  {m} <button onClick={() => setForm(p => ({ ...p, members: p.members.filter(x => x !== m) }))} className="text-slate-500 hover:text-white">×</button>
                </span>
              ))}
            </div>
          </div>
